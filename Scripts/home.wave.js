function Wave() {
	...
	var FRICTION = 1.14;
	var MOUSE_PULL = 0.09;
	// The strength at which the mouse pulls particles within the AOE
	var AOE = 200;
	// Area of effect for mouse pull
	var DETAIL = Math.round(WIDTH / 60);
	// The number of particles used to build up the wave
	var WATER_DENSITY = 1.07;
	var AIR_DENSITY = 1.02;
	var TWITCH_INTERVAL = 2000;
	// The interval between random impulses being inserted into the wave to keep it moving

	/** Twitter interaction settings */
	var TWITTER_QUERY = 'water';
	// The search term for tweets
	var TWEETS_PER_PAGE = 20;
	// The number of tweets that will be fetched per server call
	var TWEETS_FREQUENCY = 400;
	// Milliseconds between tweet bubbles being added to the wave

	/** Bubble settings */
	var MAX_BUBBLES = 60;
	// The maximum number of bubbles visible before FIFO is applied
	var BIG_BUBBLE_DISSOLVE = 20;
	// How many particles a bubble dissolves into when being clicked
	var SMALL_BUBBLE_DISSOLVE = 6;

	/** Couple of counters to keep track of the Twitter interaction */
	var twitterPolls = 1;
	// Counts how many times data has been fetched from twitter
	var twitterRetries = 0;
	// Counts how many times we fail to retrieve results from twitter
	var twitterMaxId = 0;
	// The ID of the oldest tweet that's been downloaded
	...
	var features = ['Microsoft Certified Soluctions Developer', 'Development using LAMP & Microsoft technologies', 'Services IT & Consulting', 'Innovation & Investigation', 'Expert Experience on Web environment', 'HTML5 & CSS3 & Javascript', 'Research & Testing on the latest technologies', 'LINUX & PHP & MySQL', 'Cloud Computing - Windows Azure', 'Analysis & review of the customer needs'];
	var indexFeature = 0;
	var totalFeaturesItems = 9;
	...
	function ShowTweet(bubbleIndex) {
		...
		var tweetMarkup = "<div class='feature'>" + "{ " + features[indexFeature] + " }" + "</div>";
		$("#tweet").hide().html(tweetMarkup).fadeIn('slow');

		indexFeature = (indexFeature == totalFeaturesItems) ? 0 : indexFeature + 1;
	}
	...
	function TimeUpdate(e) {
		...
		gradientFill.addColorStop(1, 'rgba(200,200,250,0)');
		...
		for ( i = 0; i < len; i++) {
			...
			if (b.x > WIDTH - b.currentSize)
				b.velocity.x = -b.velocity.x;
			if (b.x < b.currentSize)
			...
			if (b.dissolved == false) {
				...
				for (var j = 0; j < b.children.length; j++) {
					...
					context.moveTo(b.x + c.x, b.y + c.y);
					// needed in ff
